...
class Interface : protected Guest, protected Admin, protected Test
{
	Admin admin;
public:
	Interface() {}
				...
				if (LoginOptions == 1) {
					...
					while (true) {
						do
						{
							...
							cin.ignore();
							cout << "\n";
							if (AdminOptions < 1 || AdminOptions > 2) {
								system("cls");
								cout << "Please input 1 or 2!\n"; 
							}
						} while (AdminOptions < 1 || AdminOptions > 2);
					...
					while (true) {
						...
						if (AdminMenuOptions == 1) {
							...
							do
							{
								...
								IsPassChanged = AdminChangePass(FormerPass, NewPass, Login);
								if (!IsPassChanged) {
									cout << "Input correct data!\n";
									cout << "\nIf you want to go back, then instead of the login, you will enter zero!\n";
									FormerPass[0] = '\n';
